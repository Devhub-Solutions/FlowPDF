            ...
            {tabs.map((t) => {
              ...
              return (
                <Link
                  key={t.to}
                  to={t.to}
                  className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-sm transition-all font-medium ${active ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900'}`}
                >
                  {t.icon} {t.label}
                </Link>
              );
